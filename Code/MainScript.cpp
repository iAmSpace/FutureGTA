...
namespace Big
{
	...
	void MainScript::Init() {
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Home", SubmenuHome, [](RegularSubmenu* sub) {
			...
			sub->AddOption<RegularOption>("Spawn an Adder", "Spawns an adder nearby.", [] {
				JobStart(= ) {
					Hash ModelHash = MISC::GET_HASH_KEY("adder");
					while (!STREAMING::HAS_MODEL_LOADED(ModelHash)) {
						STREAMING::REQUEST_MODEL(ModelHash);
						Script::GetCurrent()->ScriptYield(10ms);
					}
					NativeVector3 VectorPos = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true);
					Vehicle CreatedVehicle = VEHICLE::CREATE_VEHICLE(ModelHash, VectorPos.x, VectorPos.y, VectorPos.z, 0.f, TRUE, FALSE, FALSE);
					if (NETWORK::NETWORK_IS_SESSION_ACTIVE()) {
						DECORATOR::DECOR_SET_INT(CreatedVehicle, "MPBitset", 0);
						DECORATOR::DECOR_SET_INT(CreatedVehicle, "Player_Vehicle", NETWORK::NETWORK_HASH_FROM_PLAYER_HANDLE(PLAYER::PLAYER_PED_ID()));
						ENTITY::_SET_ENTITY_SOMETHING(CreatedVehicle, TRUE);
						int NetworkID = NETWORK::VEH_TO_NET(CreatedVehicle);
						if (NETWORK::NETWORK_GET_ENTITY_IS_NETWORKED(CreatedVehicle)) NETWORK::SET_NETWORK_ID_EXISTS_ON_ALL_MACHINES(NetworkID, true);
						VEHICLE::SET_VEHICLE_IS_STOLEN(CreatedVehicle, FALSE);
					}
					VEHICLE::SET_VEHICLE_ENGINE_ON(CreatedVehicle, true, true, true);
					if (VEHICLE::IS_THIS_MODEL_A_HELI(ModelHash) || VEHICLE::IS_THIS_MODEL_A_PLANE(ModelHash))
						VEHICLE::SET_HELI_BLADES_FULL_SPEED(CreatedVehicle);
					if (VEHICLE::IS_THIS_MODEL_A_PLANE(ModelHash)) {
						VEHICLE::_SET_VEHICLE_JET_ENGINE_ON(CreatedVehicle, true);
						VEHICLE::SET_VEHICLE_FORWARD_SPEED(CreatedVehicle, 40);
					}
					PED::SET_PED_INTO_VEHICLE(PLAYER::PLAYER_PED_ID(), CreatedVehicle, -1);
					VEHICLE::SET_VEHICLE_MOD_KIT(CreatedVehicle, 0);
					for (int i = 0; i < 49; i++) {
						if (i > 16 && i < 23) continue;
						VEHICLE::SET_VEHICLE_MOD(CreatedVehicle, i, VEHICLE::GET_NUM_VEHICLE_MODS(CreatedVehicle, i) - 1, false);
					}
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Demo_sub", SubmenuTest, [](RegularSubmenu* sub) {
			sub->AddOption<RegularOption>("Option", "A regular option.", [] {
				g_Logger->Info("You pressed the test option");
			});
			static bool testBool1{};
			sub->AddOption<BoolOption<bool>>("Bool (On/Off)", nullptr, &testBool1, BoolDisplay::OnOff);
			static bool testBool2{};
			sub->AddOption<BoolOption<bool>>("Bool (Yes/No)", nullptr, &testBool2, BoolDisplay::YesNo);
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Header", SubmenuSettingsHeader, [](RegularSubmenu* sub) {
			...
			switch (g_UiManager->m_HeaderType) {
				case HeaderType::Static: sub->AddOption<SubOption>("Background", nullptr, SubmenuSettingsHeaderStaticBackground); break;
				case HeaderType::Gradient: sub->AddOption<SubOption>("Gradient", nullptr, SubmenuSettingsHeaderGradientBackground); break;
			}
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Players", SubmenuPlayerList, [](RegularSubmenu* sub) {
			for (std::uint32_t i = 0; i < 32; ++i) {
				if (auto ped = PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(i)) {
					sub->AddOption<SubOption>(PLAYER::GET_PLAYER_NAME(i), nullptr, SubmenuSelectedPlayer, [=] { g_SelectedPlayer = i; });
				}
			}
		});
		g_UiManager->AddSubmenu<PlayerSubmenu>(&g_SelectedPlayer, SubmenuSelectedPlayer, [](PlayerSubmenu* sub) {
			sub->AddOption<RegularOption>("Placeholder Option", nullptr);
		});
	}
